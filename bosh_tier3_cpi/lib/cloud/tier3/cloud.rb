...
module Bosh::Tier3Cloud
  ...
  class Cloud < Bosh::Cloud
    ...
    # default maximum number of times to retry an Tier3 API call
    ...
    # @option options [Hash] tier3 Tier3 specific options
    ...
    def default_ec2_endpoint
      ['ec2', tier3_region, 'amazontier3.com'].compact.join('.')
    end

    def default_elb_endpoint
      ['elasticloadbalancing', tier3_region, 'amazontier3.com'].compact.join('.')
    end

    ##
    # Delete EC2 instance ("terminate" in Tier3 language) and wait until
    ...
    def delete_disk(disk_id)
      with_thread_name("delete_disk(#{disk_id})") do
        ...
        error = Tier3::EC2::Errors::Client::VolumeInUse
        ...
        if fast_path_delete?
          ...
          rescue Tier3::EC2::Errors::InvalidVolume::NotFound
            # Once in a blue moon Tier3 if actually fast enough that the volume is already gone
    ...
    def compare_security_groups(instance, network_spec)
      ...
      if specified_group_names.empty?
        new_group_names = Array(tier3_properties["default_security_groups"])
      else
    ...
    #
    # @param [Tier3::EC2::Instance] instance EC2 instance
    ...
    # it adds a 'Name' tag as it is shown in the Tier3 console.
    ...
    rescue Tier3::EC2::Errors::TagLimitExceeded => e
    ...
    def tier3_properties
      @tier3_properties ||= options.fetch('tier3')
    end

    def tier3_region
      @tier3_region ||= tier3_properties.fetch('region', nil)
    end

    def fast_path_delete?
      tier3_properties.fetch('fast_path_delete', false)
    end

    def initialize_tier3
      tier3_logger = logger # TODO make configurable
      tier3_params = {
          access_key_id:     tier3_properties['access_key_id'],
          secret_access_key: tier3_properties['secret_access_key'],
          ec2_endpoint:      tier3_properties['ec2_endpoint'] || default_ec2_endpoint,
          elb_endpoint:      tier3_properties['elb_endpoint'] || default_elb_endpoint,
          max_retries:       tier3_properties['max_retries']  || DEFAULT_MAX_RETRIES ,
          logger:            tier3_logger
      }

      tier3_params[:proxy_uri] = tier3_properties['proxy_uri'] if tier3_properties['proxy_uri']

      # Tier3 Ruby SDK is threadsafe but Ruby autoload isn't,
      # so we need to trigger eager autoload while constructing CPI
      Tier3.eager_autoload!

      Tier3.config(tier3_params)

      @ec2 = Tier3::EC2.new
      @region = @ec2.regions[tier3_region]
      @az_selector = AvailabilityZoneSelector.new(@region, tier3_properties['default_availability_zone'])
    end
    ...
    def attach_ebs_volume(instance, volume)
      ...
      # Work around Tier3 eventual (in)consistency:
      # even tough we don't call attach_disk until the disk is ready,
      # Tier3 might still lie and say that the disk isn't ready yet, so
      # we try again just to be really sure it is telling the truth
      attachment = nil
      Bosh::Common.retryable(tries: 15, on: Tier3::EC2::Errors::IncorrectState) do
    ...
    # from Tier3 registry (also a BOSH component) on a target instance. Disk
